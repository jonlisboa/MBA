...
RL_FIRM          <- 0.006   # baseline firm lending rate
RL_PSI           <- 0.08    # rate sensitivity to capital shortfall (Lavoie 2014 ch.4)
                             # RL_t = RL_FIRM + RL_PSI * max(0,(CAR_MIN - CAR_t)/CAR_MIN)
RD               <- 0.0015
CAR_MIN          <- 0.08
LOAN_MATURITY    <- 60L
...
# Taxes & distribution
PAYROLL_TAX   <- 0.1 #from 0.20
PROFIT_TAX    <- 0.27 #from 0.27
WEALTH_FLOOR  <- 10
INCOME_FRAC   <- 0.8
RL_GOV        <- 0.005
BF_MEDIAN_FRAC  <- 0.45
...
FISCAL_EWMA_TAU   <- 20
G_SHARE_INIT      <- 0.6 #from 0.6
G_SHARE_MIN       <- 0.40 #from 0.55
G_SHARE_MAX       <- 0.70 #from 0.80
G_SHARE_PHI       <- 0.15
M_LG_TARGET_MULT  <- 6
...
# Pre-compute the minimum d_hf size this run requires, accounting for
# population growth over the full horizon. Used both to pre-allocate on a
# fresh build and to validate a loaded cache (the cache name only encodes
# N_HH, so a changed POP_GROWTH_RATE or MAXT would otherwise go undetected).
N_MAX_NEEDED <- ceiling(N_HH * (1 + POP_GROWTH_RATE)^MAXT * 1.05) + MAXT
...
if (QUICK_STARTUP && file.exists(CACHE_FILE)) {
  ...
  # Validate: the cached d_hf must be large enough for the current run's
  # population growth trajectory. If the cache was built with a different
  # POP_GROWTH_RATE or MAXT (the filename only encodes N_HH), the matrix
  # may be undersized, causing a subscript-out-of-bounds crash mid-run.
  if (N_MAX < N_MAX_NEEDED) {
    cat(sprintf("  [WARNING] Cached d_hf (%dx%d) is too small for current parameters",
                N_MAX, N_MAX))
    cat(sprintf(" (need at least %d). Discarding stale cache — rebuilding.\n", N_MAX_NEEDED))
    rm(road_pts, hh_x, hh_y, d_hf)
    # cache_loaded stays FALSE; fall through to fresh build below
  } else {
    cache_loaded <- TRUE
...
lg <- list(M = init_gov_M, L = 0, tax_rev = 0, transfer = 0,
...
for (period in seq_len(MAXT)) {
  ...
  if (POP_GROWTH_RATE > 0 && period > 1L) {
    ...
    # Guard: expand d_hf if the pre-allocated matrix is exhausted.
    # Under normal operation this should not trigger (the cache validation
    # above ensures d_hf is large enough). It is a last-resort safeguard
    # against any remaining edge case (e.g. rounding, parameter mid-run change).
    needed_size <- N_HH_old + n_births
    if (needed_size > nrow(d_hf)) {
      expand_to <- needed_size + max(n_births * 10L, 200L)
      old_size  <- nrow(d_hf)
      new_dhf   <- matrix(0.0, expand_to, expand_to)
      new_dhf[seq_len(old_size), seq_len(old_size)] <- d_hf
      d_hf  <- new_dhf
      N_MAX <- expand_to
      warning(sprintf(
        "Period %d: d_hf expanded from %dx%d to %dx%d. Delete the spatial cache to avoid this.",
        period, old_size, old_size, expand_to, expand_to))
    }
  ...
  # Endogenous lending rate (Lavoie 2014 ch.4; Godley & Lavoie 2007 ch.10)
  # Bank marks up the baseline rate when its CAR falls below the minimum,
  # recovering margin and accelerating exit of Ponzi borrowers simultaneously.
  # CAR_t = bank_NW / sum(f_L); shortfall = max(0, CAR_MIN - CAR_t) / CAR_MIN
  {
    total_f_L_now <- sum(f_L[om])
    CAR_t         <- if (total_f_L_now > 0) bank_NW / total_f_L_now else CAR_MIN
    car_shortfall <- max(0, (CAR_MIN - CAR_t) / CAR_MIN)
    RL_t          <- RL_FIRM + RL_PSI * car_shortfall
  }
  ...
  # Step 2: loan interest (firm loans) — rate is endogenous RL_t
  f_Int[]   <- 0; f_Int[om] <- f_L[om] * RL_t
  ...
  # Step 4a: mandatory overdraft closure (SFC-preserving, no CAR limit)
  # Any firm with f_M < 0 after wages and interest gets a forced loan to reach
  # exactly zero. Bank creates deposit + loan symmetrically: ΔD = ΔL, ΔR = 0.
  # Equivalent to a guaranteed intraperiod overdraft. Ponzi counter still fires
  # on interest non-coverage, so credit discipline is preserved.
  neg_idx <- which(om & f_M < 0)
  for (fi in neg_idx) {
    overdraft      <- -f_M[fi]
    f_L[fi]        <- f_L[fi] + overdraft
    f_M[fi]        <- 0
    bank_new_loans <- bank_new_loans + overdraft
  }
